...
export const ContentView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch("/api/content-items")
      ...
      .catch((err) => console.error("Error fetching live content items:", err));
    return () => {
      isMounted = false;
    };
  }, []);
  ...
  const handleSync = async () => {
    ...
    try {
      const res = await fetch("/api/content-items", {
        ...
        body: JSON.stringify({ refreshLive: true }),
      });
      const json = await res.json();
      if (json.success && Array.isArray(json.items)) {
        setContentList(json.items);
        setLastSyncTime(new Date().toLocaleTimeString("id-ID"));
        setSyncStatus("Data live Instagram & spreadsheet berhasil disinkronkan!");
        ...
        <div>
          <div className="flex items-center gap-2 text-caption font-medium uppercase tracking-wider text-foreground-secondary mb-1">
            ...
            <span className="inline-flex items-center gap-1 text-emerald-800 font-semibold bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
              ...
              Live Instagram Sync Terverifikasi
            </span>
            {lastSyncTime && (
